...
class AppStore extends Component {
  state = {uniqueTabId: tabsList[0].tabId, inputSearch: ''}

  filteredTab = () => {
    const {uniqueTabId} = this.state

    const selectedTab = appsList.filter(
      eachapp => eachapp.category === uniqueTabId,
    )
  ...
  render() {
    const {uniqueTabId, inputSearch} = this.state

    const selectedTab = this.filteredTab()

    const filteredApps = selectedTab.filter(eachAp =>
      eachAp.appName.toLowerCase().includes(inputSearch.toLowerCase()),
    )
    ...
    return (
      <div className="bgContainer">
        <diV className="bgCont">
          ...
          <ul className="unordedList-Tabs">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                eachTab={eachTab}
                isTabClicked={this.isTabClicked}
                isActive={eachTab.tabId === uniqueTabId}
              />
            ))}
          </ul>

          <ul className="unordedList-apps">
